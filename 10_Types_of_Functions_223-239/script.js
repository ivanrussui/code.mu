...
//
//     В простых случаях это не составляет труда сделать визуально. Но как проверить, что вы сделали это правильно?
//     Используйте разницу между Function Declaration или Function Expression:
//     первые могут быть вызваны выше своего определения, а вторые - нет.
//
...
//     console.log('!');
// }
// Так как функция выше - это Function Expression и она не присвоена никакой переменной, то ее никак не вызвать,
...
//         console.log('!');
//     }
// Как вы уже знаете, эта функция является функциональным выражением, ни смотря на то, что у нее задано имя
// (мы уже выяснили, что наличие имени вообще не критерий). Уберем этот плюс - и получим Function Declaration:
//
//     function func() {
//         console.log('!');
//     }
...
//     console.log('!');
// }
// Почему так: потому что на первой строке написана одна законченная команда, закрытая точкой с запятой.
// Поэтому эта команда никак не влияет на нашу функцию.
//
//     На самом деле точку с запятой можно убрать, ведь в JavaScript она не обязательна - функция все равно останется
...
//         console.log('!');
//     }
// Почему так: потому что на первой строчке незавершенное выражение - стоит плюс и после него ничего.
// Поэтому интерпретатор JavaScript считает, что этот плюс относится к строке ниже, то есть к нашей функции.
//
//     Если же на первой строке стоит завершенное выражение, то JavaScript автоматически ставит ему точку с запятой и
//     это выражение никак не влияет на нашу функцию.


// 236 Выражение справа
//
// Учтите, что выражение, в котором участвует функция, должно быть слева от нее. Если мы что-то пытаемся сделать справа
...
// 237 Массив с функциями
//
// Я уже упоминал ранее, что функции в JavaScript ведут себя, будто строки или числа. В частности, можно сделать массив,
// состоящий из функций. Сделаем:
//
// let arr = [
...
//
// Чтобы происходил вызов функции, к ней нужно дописать круглые скобки. Так как наша функция хранится в arr[0],
// то круглые скобки нужно будет написать после квадратных, вот так: arr[0](). Проверим:
//
// let arr = [
// 	function() {console.log('1')},
// 	function() {console.log('2')},
...
// Объект с функциями
//
// В предыдущем уроке мы с вами делали массив, состоящий из функций.
// Давайте теперь вместо массива сделаем объект с ключами, значениями которых будут функции:
//
//     let obj = {
...
// 1 Сделайте объект с тремя функциями. Пусть первая возвращает через return число 1, вторая - число 2, третья - число 3.
...
// 239 Применение объекта с функциями
//
// Давайте сделаем объект с двумя функциями, каждая из которых параметром будет принимать число.
//     Пусть первая функция возводит переданное число в квадрат, а вторая функция - в куб. Реализуем:
//
// let math = {
...
// 1 Сделайте объект с тремя функциями, каждая из которых будет принимать параметром массив с числами. Сделайте так,
...
// console.log(obj.sqrt([5, 5]));
// console.log(obj.cube([5, 5]));
